...
[`paistr()`](https://paithiov909.github.io/shikakusphere/reference/paistr.md)は、こうした手牌の文字列表現を扱いやすくするためのS3クラス（`<skksph_paistr>`）です。[`print()`](https://rdrr.io/r/base/print.html)したときに、その文字列に含まれる牌の枚数を表示します。また、`plot(<skksph_paistr>)`で牌姿を画像としてプロットできます。
...
`<skksph_paistr>`を[`print()`](https://rdrr.io/r/base/print.html)したときに表示される牌の枚数は不正確な場合があります。手牌の表現にどの牌が何枚含まれるかを正確に確認するには、`tidy(<skksph_paistr>)`を使うことができます。
...
このかたちの表現は[`lineup()`](https://paithiov909.github.io/shikakusphere/reference/lineup.md)でlist
of factorsにすることができます。
...
characters）は[`lipai()`](https://paithiov909.github.io/shikakusphere/reference/lipai.md)で文字列表現に変換できます（ただし、[`lipai()`](https://paithiov909.github.io/shikakusphere/reference/lipai.md)では`_*+=-,`は扱えません）。
...
[`calc_defen()`](https://paithiov909.github.io/shikakusphere/reference/calc_defen.md)はvectorizeされていないため、必ず、長さが1の文字列表現を渡してください。
...
この関数は、天鳳のJSON形式を一度MJAIフォーマットのJSON文字列へと変換したうえでパースするため、牌の表現などがshikakusphereで用いているものとは異なっています。MJAIフォーマットにおける牌の表現をshikakusphereで用いている表現に変換するには、[`trans_tile()`](https://paithiov909.github.io/shikakusphere/reference/trans_tile.md),
[`mjai_target()`](https://paithiov909.github.io/shikakusphere/reference/mjai.md),
[`mjai_conv()`](https://paithiov909.github.io/shikakusphere/reference/mjai.md)を使って、次のようにします。
...
[`proceed()`](https://paithiov909.github.io/shikakusphere/reference/proceed.md)を使うと、ゲームのある時点におけるプレイヤーの手牌の状態を再現することができます。この関数には、各プレイヤーの配牌とその時点までのツモ・打牌をそれぞれlistとして与えます。チー・ポンと大明槓についてはツモの一部、暗槓と加槓については打牌の一部とします。
