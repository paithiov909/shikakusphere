...
Game::Game(const Rule& rule, const bool paipu)
    : _rule{rule}, _paipu{}, _status{Status::NONE} {
  if (paipu) {
...
void Game::set(const std::array<Shoupai, 4>& shoupai,
               const std::array<He, 4>& he, const Shan& shan,
               const int lunban) {
  _model.shan = shan;
  for (int l = 0; l < 4; l++) {
  ...
  _diyizimo = he[0].pai().size() <= 1 && he[1].pai().size() <= 1 &&
              he[2].pai().size() <= 1 && he[3].pai().size() == 0;

  _dapai = _model.lunban >= 0 && he[_model.lunban].pai().size() > 0
               ? he[_model.lunban].pai().back()
               : "";
  _fulou.clear();
  _gang.clear();

  _lizhi = {shoupai[0].lizhi(), shoupai[1].lizhi(), shoupai[2].lizhi(),
            shoupai[3].lizhi()};
  _yifa = {
  ...
  if (_paipu) {
    if (_status != Status::QIPAI) _paipu->rounds.emplace_back();
...
void Game::set(const Paipu::Round& round) {
  set(round.model.shoupai, round.model.he, round.model.shan,
      round.model.lunban);
...
void Game::qipai() { qipai_(Shan(_rule)); }
...
void Game::dapai(const std::string& p) {
  ...
  if (!_model.shoupai[_model.lunban].lizhi()) _neng_rong[_model.lunban] = true;
  ...
  if (xiangting(_model.shoupai[_model.lunban]) == 0) {
    if (find_if(tingpai(_model.shoupai[_model.lunban], xiangting),
...
void Game::gang(const std::string& m) {
  ...
  if (_paipu) _paipu->rounds.back().moves.emplace_back(Reply{Message::GANG, m});
...
void Game::hule() {
  ...
  auto rongpai = menfeng == _model.lunban
                     ? std::string{}
  ...
  auto libaopai =
      shoupai.lizhi() ? _model.shan.libaopai() : std::vector<std::string>{};
  ...
  Param param = {
      ...
      menfeng,
      Param::Hupai{
          _lizhi[menfeng], _yifa[menfeng],
          _hule_option == HuleOption::QIANGGANG,
          _hule_option == HuleOption::LINGSHANG,
          _model.shan.paishu() > 0 || _hule_option == HuleOption::LINGSHANG ? 0
          : rongpai.empty()                                                 ? 1
                                                                            : 2,
          !(_diyizimo && rongpai.empty()) ? 0
          : menfeng == 0                  ? 1
                                          : 2},
      _model.shan.baopai(),
      libaopai,
      Param::Jicun{_model.changbang, _model.lizhibang}};
  ...
  if (_paipu)
    _paipu->rounds.back().moves.emplace_back(
        Reply{Game::Message::HULE, rongpai});
  ...
  if (name == Pingju::NONE) {
    ...
    for (int l = 0; l < 4; l++) {
      ...
      if (!_rule.notenPenalty /*ノーテン罰あり*/
        ...
        shoupai[l].clear();
                 ...
                 tingpai(_model.shoupai[l], xiangting).size() > 0) {
        n_tingpai++;
    ...
    if (name == Pingju::NONE) {
      ...
      if (_rule.notenPenalty /*ノーテン罰あり*/
        ...
        for (int l = 0; l < 4; l++) {
          fenpei[l] =
              !shoupai[l].empty() ? 3000 / n_tingpai : -3000 / (4 - n_tingpai);
        }
      }
    }
...
void Game::last() {
  ...
  if (15 < sum_jushu)
    jieju_ = true;
  else if ((_rule.roundsType /*場数*/ + 1) * 4 - 1 < sum_jushu)
    jieju_ = true;
  else if (_max_jushu < sum_jushu) {
    if (_rule.overtimeType /*延長戦方式*/ == 0)
      jieju_ = true;
    else if (_rule.roundsType /*場数*/ == 0)
      jieju_ = true;
    else if (guanjun >= 0)
      jieju_ = true;
    else {
    ...
    if (_rule.lastRoundStop /*オーラス止めあり*/ &&
        guanjun == _model.player_id[0] && _lianzhuang && !_no_game)
      jieju_ = true;
  }
...
void Game::jieju() {
  ...
  for (int i = 1; i < 4; i++) {
    int id = paiming[i];
    _point[id] = (defen[id] - 30000) / 1000.0f + _rule.rankPoints /*順位点*/[i];
...
// 结局(終局)の応答
void Game::reply_kaiju() { qipai(); }

// 起牌(配牌)の応答
void Game::reply_qipai() { zimo(); }
...
void Game::reply_dapai() {
  for (int i = 1; i < 4; i++) {
    ...
    if (reply.msg == Message::HULE && allow_hule(l)) {
      ...
      shoupai.zimo(_dapai);
      if (xiangting(shoupai) == -1) _neng_rong[l] = false;
    }
  }
  if (_hule.size() == 3 &&
      _rule.maxSimultaneousWinners /*最大同時和了数*/ == 2) {
    std::array<std::string, 4> shoupai{};
  ...
  if (_dapai.back() == '*') {
    ...
    if (std::count_if(_lizhi.begin(), _lizhi.end(),
                      [](const auto x) { return x > 0; }) == 4 &&
        _rule.abortiveDraw /*途中流局あり*/) {
      std::array<std::string, 4> shoupai;
      std::transform(_model.shoupai.begin(), _model.shoupai.end(),
                     shoupai.begin(),
  ...
  if (std::accumulate(_n_gang.begin(), _n_gang.end(), 0) == 4) {
    if (_rule.abortiveDraw /*途中流局あり*/ &&
        *std::max_element(_n_gang.begin(), _n_gang.end()) < 4) {
...
std::vector<std::string> Game::get_chi_mianzi(const int l) const {
  ...
  return Game::_get_chi_mianzi(_rule, _model.shoupai[l], _dapai + d,
                               _model.shan.paishu());
}
...
std::vector<std::string> Game::get_peng_mianzi(const int l) const {
  const auto d = "_+=-"[(4 + _model.lunban - l) % 4];
  return Game::_get_peng_mianzi(_rule, _model.shoupai[l], _dapai + d,
                                _model.shan.paishu());
}
...
std::vector<std::string> Game::get_gang_mianzi(const int l) const {
  if (l < 0) {
    return Game::_get_gang_mianzi(
        _rule, _model.shoupai[_model.lunban], {}, _model.shan.paishu(),
        std::accumulate(_n_gang.begin(), _n_gang.end(), 0));
  } else {
    const auto d = "_+=-"[(4 + _model.lunban - l) % 4];
    return Game::_get_gang_mianzi(
        _rule, _model.shoupai[l], _dapai + d, _model.shan.paishu(),
        std::accumulate(_n_gang.begin(), _n_gang.end(), 0));
  }
}
...
std::pair<bool, std::vector<std::string>> Game::allow_lizhi(
  ...
  return Game::_allow_lizhi(_rule, _model.shoupai[_model.lunban], p,
                            _model.shan.paishu(),
...
bool Game::allow_hule(const int l) const {
  if (l < 0) {
    const auto hupai = _model.shoupai[_model.lunban].lizhi() ||
                       _status == Status::GANGZIMO || _model.shan.paishu() == 0;
    return Game::_allow_hule(_rule, _model.shoupai[_model.lunban], {},
                             _model.zhuangfeng, _model.lunban, hupai);
  } else {
    const auto p =
        (_status == Status::GANG ? _gang.substr(0, 1) + _gang.back() : _dapai) +
        "_+=-"[(4 + _model.lunban - l) % 4];
    const auto hupai = _model.shoupai[l].lizhi() || _status == Status::GANG ||
                       _model.shan.paishu() == 0;
    return Game::_allow_hule(_rule, _model.shoupai[l], p, _model.zhuangfeng, l,
                             hupai, _neng_rong[l]);
  }
}
...
bool Game::allow_pingju() const {
  return Game::_allow_pingju(_rule, _model.shoupai[_model.lunban], _diyizimo);
}

// 打牌取得
std::vector<std::string> Game::_get_dapai(const Rule& rule,
                                          const Shoupai& shoupai) {
  if (rule.canChangePermissionLevel /*喰い替え許可レベル*/ == 0)
...
// 吃(チー)面子取得
std::vector<std::string> Game::_get_chi_mianzi(const Rule& rule,
                                               const Shoupai& shoupai,
                                               const std::string& p,
                                               const int paishu) {
  auto mianzi = shoupai.get_chi_mianzi(
      p, rule.canChangePermissionLevel /*喰い替え許可レベル*/ == 0);
  if (mianzi.empty()) return mianzi;
  if (rule.canChangePermissionLevel /*喰い替え許可レベル*/ == 1 &&
      shoupai.fulou_().size() == 3 && shoupai.bingpai(p[0])[to_int(p[1])] == 2)
    mianzi.clear();
  return paishu == 0 ? std::vector<std::string>{} : mianzi;
}

// 碰(ポン)面子取得
std::vector<std::string> Game::_get_peng_mianzi(const Rule& rule,
                                                const Shoupai& shoupai,
                                                const std::string& p,
                                                const int paishu) {
...
// 杠(槓)面子取得
std::vector<std::string> Game::_get_gang_mianzi(const Rule& rule,
                                                const Shoupai& shoupai,
                                                const std::string& p,
                                                const int paishu,
                                                const int n_gang) {
  auto mianzi =
      p.empty() ? shoupai.get_gang_mianzi() : shoupai.get_gang_mianzi(p);
  if (mianzi.empty()) return mianzi;

  if (shoupai.lizhi()) {
    if (rule.lizhiPostClosedGangPermissionLevel /*リーチ後暗槓許可レベル*/ == 0)
      return {};
    else if (rule.lizhiPostClosedGangPermissionLevel /*リーチ後暗槓許可レベル*/
             == 1) {
      int n_hule1 = 0, n_hule2 = 0;
      auto new_shoupai = shoupai;
      new_shoupai.dapai(shoupai.zimo_());
      for (const auto& p : tingpai(new_shoupai, xiangting)) {
      ...
      auto new_shoupai = shoupai;
      new_shoupai.dapai(shoupai.zimo_());
      int n_tingpai1 = (int)tingpai(new_shoupai, xiangting).size();
      new_shoupai = shoupai;
      new_shoupai.gang(mianzi[0]);
      if (xiangting(new_shoupai) > 0) return {};
      int n_tingpai2 = (int)tingpai(new_shoupai, xiangting).size();
...
// 立直可能
std::pair<bool, std::vector<std::string>> Game::_allow_lizhi(
    const Rule& rule, const Shoupai& shoupai, const std::string& p,
    const int paishu, const int defen) {
  if (shoupai.zimo_().empty()) return {false, {}};
  if (shoupai.lizhi()) return {false, {}};
  if (!shoupai.menqian()) return {false, {}};

  if (!rule.lizhiWithoutTsumoBonus /*ツモ番なしリーチあり*/ && paishu < 4)
    return {false, {}};
  if (rule.bankruptcyEndAll /*トビ終了あり*/ && defen < 1000)
    return {false, {}};
  ...
  if (!p.empty()) {
    ...
    return {xiangting(new_shoupai) == 0 &&
                tingpai(new_shoupai, xiangting).size() > 0,
            {}};
  } else {
...
// 和了可能
bool Game::_allow_hule(const Rule& rule, const Shoupai& shoupai,
                       const std::string& p, const int zhuangfeng,
                       const int menfeng, const bool hupai,
                       const bool neng_rong) {
...
// 流局可能
bool Game::_allow_pingju(const Rule& rule, const Shoupai& shoupai,
                         const bool diyizimo) {
...
// ノーテン宣言可能
bool Game::_allow_no_daopai(const Rule& rule, const Shoupai& shoupai,
                            const int paishu) {
  if (paishu > 0 || !shoupai.zimo_().empty()) return false;
  if (!rule.notenDeclaration /*ノーテン宣言あり*/) return false;
  if (shoupai.lizhi()) return false;

  return xiangting(shoupai) == 0 && tingpai(shoupai, xiangting).size() > 0;
}
