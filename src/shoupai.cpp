...
Shoupai::Shoupai() : __{}, _m(10), _p(10), _s(10), _z(8), _lizhi{false} {}
...
Shoupai::Shoupai(const std::vector<std::string>& pai,
                 const std::vector<std::string>& fulou)
    : __{}, _m(10), _p(10), _s(10), _z(8), _lizhi{false}, _fulou{fulou} {
  _set(pai);
  if (pai.size() + fulou.size() * 3 == 14) _zimo = pai.back();
}
...
Shoupai& Shoupai::zimo(const std::string& p, bool check) {
  ...
  if (p == "_") {
    ...
    auto& bingpai = _bingpai(s);
    if (bingpai[n] == 4)
      throw std::invalid_argument(p + " cannot be greater than 4");
    bingpai[n]++;
    if (n == 0) {
      if (bingpai[5] == 4)
        throw std::invalid_argument(p.substr(0, 1) +
                                    "5 cannot be greater than 4");
      bingpai[5]++;
    }
    _zimo = p.substr(0, 2);
  }
  ...
  if (!_lizhi) {
    for (const auto s : {'m', 'p', 's', 'z'}) {
      ...
      for (std::size_t n = 1; n < bingpai.size(); n++) {
        ...
        if (s == 'z' || n != 5)
          dapai.emplace_back(std::move(pai));
        else {
          const char s0[]{s, '0'};
          std::string pai0(s0, 2);
          if (bingpai[0] > 0 && pai0 != _zimo || bingpai[0] > 1)
...
// 吃(チー)面子取得
std::vector<std::string> Shoupai::get_chi_mianzi(const std::string& p,
                                                 bool check) const {
  ...
  if (3 <= n && bingpai[n - 2] > 0 && bingpai[n - 1] > 0) {
    if (!check || (3 < n ? bingpai[n - 3] : 0) + bingpai[n] <
                      14 - (_fulou.size() + 1) * 3) {
      if (n - 2 == 5 && bingpai[0] > 0)
        mianzi.emplace_back(std::string(1, s) + "067-");
      if (n - 1 == 5 && bingpai[0] > 0)
        mianzi.emplace_back(std::string(1, s) + "406-");
  ...
  if (2 <= n && n <= 8 && bingpai[n - 1] > 0 && bingpai[n + 1] > 0) {
    if (!check || bingpai[n] < 14 - (_fulou.size() + 1) * 3) {
      if (n - 1 == 5 && bingpai[0] > 0)
        mianzi.emplace_back(std::string(1, s) + "06-7");
      if (n + 1 == 5 && bingpai[0] > 0)
        mianzi.emplace_back(std::string(1, s) + "34-0");
      if ((n - 1 != 5 && n + 1 != 5) || bingpai[0] < bingpai[5])
        mianzi.emplace_back(to_string(s, n - 1, p[1], d[0], n + 1));
    }
  }
  if (n <= 7 && bingpai[n + 1] > 0 && bingpai[n + 2] > 0) {
    if (!check || bingpai[n] + (n < 7 ? bingpai[n + 3] : 0) <
                      14 - (_fulou.size() + 1) * 3) {
      if (n + 1 == 5 && bingpai[0] > 0)
        mianzi.emplace_back(std::string(1, s) + "4-06");
      if (n + 2 == 5 && bingpai[0] > 0)
        mianzi.emplace_back(std::string(1, s) + "3-40");
...
std::vector<std::string> Shoupai::get_peng_mianzi(const std::string& p) const {
  ...
  if (bingpai[n] >= 2) {
    if (n == 5 && bingpai[0] >= 2)
      mianzi.emplace_back(std::string(1, s) + "00" + p[1] + d);
...
std::vector<std::string> Shoupai::get_gang_mianzi(const std::string& p) const {
  if (!p.empty()) {
    ...
    if (bingpai[n] == 3) {
      if (n == 5)
        return {s + std::string(3 - bingpai[0], '5') +
                std::string(bingpai[0], '0') + p[1] + d};
      else
        return {to_string(s, n, n, n, n) + d};
    }
    return {};
  } else {
    if (_zimo.empty()) throw std::runtime_error("zimo must not be empty");
          ...
          for (const auto& m : _fulou) {
            if (std::regex_replace(m, re_ling(), "5").substr(0, 4) ==
              ...
              if (n == 5 && bingpai[0] > 0)
                ...
                mianzi.emplace_back(m + to_char(n));
            }
          }
        }
      }
