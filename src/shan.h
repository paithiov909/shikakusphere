...
class Shan {
 public:
  ...
  static std::string zhenbaopai(const std::string& p) {
    ...
    int n = to_int(p[1]);
    if (n == 0) n = 5;
    return s == 'z' ? (n < 5 ? to_string(s, (n % 4 + 1))
                             : to_string(s, ((n - 4) % 3 + 5)))
                    : to_string(s, (n % 9 + 1));
  }
  ...
  std::vector<std::string> baopai() const {
    ...
    for (const auto& p : _baopai) {
      if (!p.empty()) baopai_.emplace_back(p);
    }
    return baopai_;
  }
  ...
  const std::vector<std::string>& libaopai_() const { return _libaopai; }
...
inline std::ostream& operator<<(std::ostream& os, const Shan& shan) {
  for (const auto& p : shan._pai) os << p;
  os << ",";
  for (const auto& p : shan._baopai) os << p;
  os << ",";
