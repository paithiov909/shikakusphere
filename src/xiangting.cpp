...
int count_quadruplet(const Shoupai& shoupai) {
  ...
  return (
      std::count(m.begin(), m.end(), 4) + std::count(p.begin(), p.end(), 4) +
      std::count(s.begin(), s.end(), 4) + std::count(z.begin(), z.end(), 4));
}
...
std::tuple<std::array<int, 3>, std::array<int, 3>> mianzi(std::vector<int> bingpai, const int n) {
  ...
  if (n <= 7 && bingpai[n] > 0 && bingpai[n + 1] > 0 && bingpai[n + 2] > 0) {
    ...
    a[0]++;
    b[0]++;
    if (a[2] < max_a[2] || (a[2] == max_a[2] && a[1] < max_a[1])) max_a = a;
    if (b[0] > max_b[0] || (b[0] == max_b[0] && b[1] > max_b[1])) max_b = b;
  }
...
int mianzi_all(const Shoupai& shoupai, const bool jiangpai) {
  ...
  for (int n = 1; n <= 7; n++) {
    if (shoupai.z_(n) >= 3)
      z[0]++;
    else if (shoupai.z_(n) == 2)
      z[1]++;
    else if (shoupai.z_(n) == 1)
      z[2]++;
  }
...
int xiangting(const Shoupai& shoupai) {
  ...
  return std::min(
      {min_yiban, xiangting_guoshi(shoupai), xiangting_qidui(shoupai)});
}

// 听牌(向聴数が減る牌)
std::vector<std::string> tingpai(const Shoupai& shoupai,
