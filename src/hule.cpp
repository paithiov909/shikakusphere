...
// 和了牌をマーク
std::vector<std::vector<std::string>> add_hulepai(
    const std::vector<std::string>& mianzi, const std::string& p) {
  const char s = p[0];
  const char n = p[1];
  const char d = p[2];
  ...
  for (std::size_t i = 0; i < mianzi.size(); i++) {
...
// 一般形(4面子1雀頭)
std::vector<std::vector<std::string>> hule_mianzi_yiban(
...
// 七対子形
std::vector<std::vector<std::string>> hule_mianzi_qidui(
    const Shoupai& shoupai, const std::string& hulepai) {
...
// 国士無双形
std::vector<std::vector<std::string>> hule_mianzi_guoshi(
    const Shoupai& shoupai, const std::string& hulepai) {
  if (shoupai.fulou_().size() > 0) return {};
...
// 九蓮宝燈形
std::vector<std::vector<std::string>> hule_mianzi_jiulian(
    const Shoupai& shoupai, const std::string& hulepai) {
  if (shoupai.fulou_().size() > 0) return {};
  ...
  return std::vector<std::vector<std::string>>{
      std::vector<std::string>{std::move(mianzi)}};
}

// 和了形
std::vector<std::vector<std::string>> hule_mianzi(const Shoupai& shoupai,
  ...
  const auto hulepai = std::regex_replace(
      rongpai.empty() ? new_shoupai.zimo_() + '_' : rongpai, re_ling(), "5");
...
struct Hudi {
  ...
  Hudi(const int zhuangfeng, const int menfeng)
...
Hudi get_hudi(const std::vector<std::string>& mianzi, const int zhuangfeng,
              const int menfeng, const Rule& rule) {
  ...
  for (const auto& m : mianzi) {
    ...
    if (m == mianzi[0]) {
      ...
      hudi.n_kezi++;
      int fu = 2;
      if (std::regex_match(m, re_yaojiu())) {
        fu *= 2;
      }
      if (std::regex_match(m, re_ankezi())) {
        fu *= 2;
        hudi.n_ankezi++;
      }
      if (std::regex_match(m, re_gangzi())) {
        fu *= 4;
        hudi.n_gangzi++;
      }
      hudi.fu += fu;
...
std::vector<Hupai> get_hupai(const std::vector<std::string>& mianzi,
                             const Hudi& hudi,
                             const std::vector<Hupai>& pre_hupai,
                             const std::vector<Hupai>& post_hupai,
                             const Rule& rule) {
  // // 門前清自摸和
  ...
  const auto fanpai = [&hudi]() {
    ...
    if (hudi.kezi_z[hudi.zhuangfeng + 1])
      fanpai_all.emplace_back(Hupai::Name(Hupai::ZHUANGFENG + hudi.zhuangfeng),
                              1);
    // 自風
  ...
  const auto sansetongshun = [&hudi]() {
    for (int n = 1; n <= 7; n++) {
      if (hudi.shunzi_m[n] && hudi.shunzi_p[n] && hudi.shunzi_s[n])
        return std::vector<Hupai>{
            Hupai{Hupai::SANSETONGSHUN, (hudi.menqian ? 2 : 1)}};
    }
    return std::vector<Hupai>{};
  };
  // 一気通貫
  const auto yiqitongguan = [&hudi]() {
    for (const auto s : {'m', 'p', 's'}) {
      if (hudi.shunzi(s)[1] && hudi.shunzi(s)[4] && hudi.shunzi(s)[7])
        return std::vector<Hupai>{
            Hupai{Hupai::YIQITONGGUAN, (hudi.menqian ? 2 : 1)}};
    }
    return std::vector<Hupai>{};
  };
  // 混全帯幺九
  const auto hunquandaiyaojiu = [&hudi]() {
    if (hudi.n_yaojiu == 5 && hudi.n_shunzi > 0 && hudi.n_zipai > 0)
      return std::vector<Hupai>{
          Hupai{Hupai::HUNQUANDAIYAOJIU, (hudi.menqian ? 2 : 1)}};
    return std::vector<Hupai>{};
  };
  // 七対子
  ...
  const auto sangangzi = [&hudi]() {
    if (hudi.n_gangzi == 3)
      return std::vector<Hupai>{Hupai{Hupai::SANGANGZI, 2}};
    return std::vector<Hupai>{};
  };
  // 三色同刻
  ...
  const auto hunlaotou = [&hudi, &mianzi]() {
    if (hudi.n_yaojiu == mianzi.size() && hudi.n_shunzi == 0 &&
        hudi.n_zipai > 0)
  ...
  const auto hunyise = [&hudi, &mianzi]() {
    for (const auto s : {'m', 'p', 's'}) {
      ...
      if (std::count_if(mianzi.begin(), mianzi.end(),
                        [&re_yise](const auto& m) {
                          return std::regex_search(m, re_yise);
                        }) == mianzi.size() &&
          hudi.n_zipai > 0)
        return std::vector<Hupai>{
            Hupai{Hupai::HUNYISE, (hudi.menqian ? 3 : 2)}};
    }
    return std::vector<Hupai>{};
  };
  // 純全帯幺九
  const auto chunquandaiyaojiu = [&hudi]() {
    if (hudi.n_yaojiu == 5 && hudi.n_shunzi > 0 && hudi.n_zipai == 0)
      return std::vector<Hupai>{
          Hupai{Hupai::CHUNQUANDAIYAOJIU, (hudi.menqian ? 3 : 2)}};
    return std::vector<Hupai>{};
  };
  // 二盃口
  ...
  const auto qingyise = [&hudi, &mianzi]() {
    for (const auto s : {'m', 'p', 's'}) {
      ...
      if (std::count_if(mianzi.begin(), mianzi.end(),
        ...
        return std::vector<Hupai>{
            Hupai{Hupai::QINGYISE, (hudi.menqian ? 6 : 5)}};
    }
    return std::vector<Hupai>{};
  };
  ...
  const auto guoshiwushuang = [&hudi, &mianzi]() {
    ...
    if (hudi.danqi)
      return std::vector<Hupai>{Hupai{Hupai::GUOSHIWUSHUANGSHISANMIAN, -2}};
    else
      return std::vector<Hupai>{Hupai{Hupai::GUOSHIWUSHUANG, -1}};
  };
  // 四暗刻
  const auto sianke = [&hudi]() {
    if (hudi.n_ankezi != 4) return std::vector<Hupai>{};
    if (hudi.danqi)
      return std::vector<Hupai>{Hupai{Hupai::SIANKEDANQI, -2}};
    else
      return std::vector<Hupai>{Hupai{Hupai::SIANKE, -1}};
  };
  // 大三元
  const auto dasanyuan = [&hudi, &mianzi]() {
    if (hudi.kezi_z[5] + hudi.kezi_z[6] + hudi.kezi_z[7] == 3) {
      ...
      std::copy_if(
          mianzi.begin(), mianzi.end(), std::back_inserter(bao_mianzi),
          [](const auto& m) { return std::regex_search(m, re_dasanyuan()); });
      std::smatch baojia;
      if (bao_mianzi.size() > 2 &&
          std::regex_search(bao_mianzi[2], baojia, re_menqian()))
        return std::vector<Hupai>{Hupai{Hupai::DASANYUAN, -1, baojia.str()[0]}};
      else
        return std::vector<Hupai>{Hupai{Hupai::DASANYUAN, -1}};
    }
    return std::vector<Hupai>{};
  };
  // 四喜和
  const auto sixihu = [&hudi, &mianzi]() {
    if (hudi.kezi_z[1] + hudi.kezi_z[2] + hudi.kezi_z[3] + hudi.kezi_z[4] ==
        4) {
      std::vector<std::string> bao_mianzi;
      std::copy_if(
          mianzi.begin(), mianzi.end(), std::back_inserter(bao_mianzi),
          [](const auto& m) { return std::regex_search(m, re_sixihu1()); });
      // 大四喜
      std::smatch baojia;
      if (bao_mianzi.size() > 3 &&
          std::regex_search(bao_mianzi[3], baojia, re_menqian()))
        return std::vector<Hupai>{Hupai{Hupai::DASIXI, -2, baojia.str()[0]}};
      else
        return std::vector<Hupai>{Hupai{Hupai::DASIXI, -2}};
    }
    if (hudi.kezi_z[1] + hudi.kezi_z[2] + hudi.kezi_z[3] + hudi.kezi_z[4] ==
            3 &&
  ...
  const auto lvyise = [&mianzi]() {
    if (std::count_if(mianzi.begin(), mianzi.end(), [](const auto& m) {
          return std::regex_search(m, re_lvyise1());
        }) > 0)
      return std::vector<Hupai>{};
    if (std::count_if(mianzi.begin(), mianzi.end(), [](const auto& m) {
          return std::regex_search(m, re_lvyise2());
        }) > 0)
      return std::vector<Hupai>{};
    if (std::count_if(mianzi.begin(), mianzi.end(), [](const auto& m) {
          return std::regex_search(m, re_lvyise3());
        }) > 0)
      return std::vector<Hupai>{};
  ...
  const auto sigangzi = [&hudi]() {
    if (hudi.n_gangzi == 4)
      return std::vector<Hupai>{Hupai{Hupai::SIGANGZI, -1}};
    return std::vector<Hupai>{};
  };
  // 九蓮宝燈
  const auto jiulianbaodeng = [&mianzi]() {
    ...
    if (std::regex_search(mianzi[0], re_jiulianbaodeng()))
      ...
      return std::vector<Hupai>{Hupai{Hupai::JIULIANBAODENG, -1}};
  };
  ...
  if (hupai.size() > 0)
    hupai.insert(hupai.end(), post_hupai.begin(), post_hupai.end());

  return hupai;
}

std::vector<Hupai> get_post_hupai(const Shoupai& shoupai,
                                  const std::string& rongpai,
  ...
  for (std::sregex_iterator it(paistr.begin(), paistr.end(), re_get_post_hupai()),
       end;
       it != end; ++it) {
  ...
  for (const auto& p : baopai) {
    ...
    for (const auto& m : suitstr) {
      ...
      int nn = 0;
      for (std::sregex_iterator it(m_.begin(), m_.end(), regexp), end;
           it != end; ++it)
        nn++;
      if (nn) n_baopai += nn;
    }
  ...
  int n_hongpai = 0;
  int nn = 0;
  for (std::sregex_iterator it(paistr.begin(), paistr.end(), re_ling()), end;
       it != end; ++it)
    nn++;
  if (nn) n_hongpai = nn;
...
// 得分(得点)
Defen get_defen(int fu, std::vector<Hupai>& hupai, const std::string& rongpai,
                const Param& param) {
  if (hupai.size() == 0) return {};

  const auto menfeng = param.menfeng;
  int fanshu{}, damanguan{}, defen{}, base{}, baojia{}, defen2{}, base2{},
      baojia2{};

  if (hupai[0].fanshu < 0) {
    fu = 0;
    // 役満の複合あり
    damanguan = !param.rule.damanguanCombination
                    ? 1
                    : std::accumulate(hupai.begin(), hupai.end(), 0,
                                      [](const int acc, const auto& h) {
                                        return acc - h.fanshu;
                                      });
    base = 8000 * damanguan;

    auto h = std::find_if(hupai.begin(), hupai.end(),
                          [](const auto& h) { return h.baojia; });
    if (h != hupai.end()) {
      baojia2 = (menfeng + (h->baojia == '+'   ? 1
                            : h->baojia == '=' ? 2
                                               : 3)) % 4;
      base2 = 8000 * std::min(-h->fanshu, damanguan);
    }
  } else {
    fanshu = std::accumulate(
        hupai.begin(), hupai.end(), 0,
        [](const int acc, const auto& h) { return acc + h.fanshu; });
  ...
  return {std::move(hupai), fu, fanshu, damanguan, defen + defen2, fenpei};
}

Defen hule(const Shoupai& shoupai, std::string rongpai, const Param& param) {
  if (!rongpai.empty()) {
    if (!std::regex_search(rongpai, re_menqian_end()))
      throw std::invalid_argument(rongpai);
  ...
  const auto& post_hupai =
      get_post_hupai(shoupai, rongpai, param.baopai, param.libaopai);

  for (const auto& mianzi : hule_mianzi(shoupai, rongpai)) {
    const auto& hudi =
        get_hudi(mianzi, param.zhuangfeng, param.menfeng, param.rule);
    ...
    if (!max.defen || rv.defen > max.defen ||
        rv.defen == max.defen && (!rv.fanshu || rv.fanshu > max.fanshu ||
