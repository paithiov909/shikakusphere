...
class Game {
 public:
  ...
  enum class HuleOption { NONE, QIANGGANG, LINGSHANG };
  ...
  void set(const std::array<Shoupai, 4>& shoupai, const std::array<He, 4>& he,
           const Shan& shan, const int lunban);
  ...
  void hule();
  // 流局
  void pingju(Pingju::Name name = Pingju::NONE,
              std::array<std::string, 4> shoupai = {});
  // 終了
  void last();
  // 结局(終局)
  ...
  // 打牌取得
  static std::vector<std::string> _get_dapai(const Rule& rule,
                                             const Shoupai& shoupai);
  // 吃(チー)面子取得
  static std::vector<std::string> _get_chi_mianzi(const Rule& rule,
                                                  const Shoupai& shoupai,
                                                  const std::string& p,
                                                  const int paishu);
  // 碰(ポン)面子取得
  static std::vector<std::string> _get_peng_mianzi(const Rule& rule,
                                                   const Shoupai& shoupai,
                                                   const std::string& p,
                                                   const int paishu);
  // 杠(槓)面子取得
  static std::vector<std::string> _get_gang_mianzi(const Rule& rule,
                                                   const Shoupai& shoupai,
                                                   const std::string& p,
                                                   const int paishu,
                                                   const int n_gang = -1);
  ...
  // 立直可能
  static std::pair<bool, std::vector<std::string>> _allow_lizhi(
      const Rule& rule, const Shoupai& shoupai, const std::string& p = {},
      const int paishu = INT_MAX, const int defen = INT_MAX);
  // 和了可能
  static bool _allow_hule(const Rule& rule, const Shoupai& shoupai,
                          const std::string& p, const int zhuangfeng,
                          const int menfeng, const bool hupai,
                          const bool neng_rong = true);
  // 流局可能
  static bool _allow_pingju(const Rule& rule, const Shoupai& shoupai,
                            const bool diyizimo);
  // ノーテン宣言可能
  static bool _allow_no_daopai(const Rule& rule, const Shoupai& shoupai,
                               const int paishu);
  ...
  void set_neng_rong(const std::array<bool, 4>& neng_rong_) {
    _neng_rong = neng_rong_;
  }
  void set_neng_rong_(const int l, const bool neng_rong_) {
    _neng_rong[l] = neng_rong_;
  }
...
template <typename T>
std::array<T, 4> order_by_player_id(const T& arr,
                                    std::array<int, 4> player_id) {
  return {arr[player_id[0]], arr[player_id[1]], arr[player_id[2]],
          arr[player_id[3]]};
}
