...
namespace {
const std::regex& re_angang_or_jiagang() {
  static const std::regex v{
      R"(^[mpsz]\d\d\d\d$|[\+\=\-]\d$)"};  // 暗槓もしくは加槓
  return v;
}
}  // namespace

PaipuReplay::PaipuReplay(const Game::Paipu& paipu)
    : _paipu{paipu},
      _game{paipu.rule, false},
      _round{0},
      _ply{0},
      _skip{false} {
  _game.kaiju();
...
void PaipuReplay::qipai() {
  ...
  if (_game.status() == Game::Status::QIPAI) {
    _game.next();
  } else if ((_paipu.rounds[_round].moves[_ply].msg == Game::Message::DAPAI) ||
             (_paipu.rounds[_round].moves[_ply].msg == Game::Message::GANG &&
              std::regex_search(_paipu.rounds[_round].moves[_ply].arg,
                                re_angang_or_jiagang())) ||
             (_paipu.rounds[_round].moves[_ply].msg == Game::Message::HULE &&
              _paipu.rounds[_round].moves[_ply].arg.empty()))
    _skip = true;
}
...
void PaipuReplay::next() {
  ...
  if (_game.status() == Game::Status::HULE ||
      _game.status() == Game::Status::PINGJU) {
    _game.next();
    if (_game.status() == Game::Status::JIEJI) return;
  }
  if (_game.status() == Game::Status::QIPAI) {
    _round++;
    _ply = 0;
  ...
  if (_game.status() != Game::Status::HULE) {
    ...
    switch (move.msg) {
      ...
      case Game::Message::DAPAI:
        assert(_game.status() == Game::Status::ZIMO ||
               _game.status() == Game::Status::GANGZIMO ||
               _game.status() == Game::Status::FULOU);
      ...
      case Game::Message::GANG:
        if (std::regex_search(move.arg, re_angang_or_jiagang())) {
          ...
          if (_paipu.rounds[_round].moves[_ply].msg != Game::Message::HULE ||
              _paipu.rounds[_round].moves[_ply].arg.empty()) {
            // 槍槓以外
            _game.next();
          }
...
std::ostream& operator<<(std::ostream& os, const Game::Model& model) {
  os << model.qijia << " " << model.zhuangfeng << " " << model.jushu << " "
     << model.changbang << " " << model.lizhibang << " " << model.defen << " "
     << model.shan << " " << model.shoupai << " " << model.he << " "
     << model.player_id << " " << model.lunban;
  return os;
}
...
std::ostream& operator<<(std::ostream& os, const Game::Paipu::Round& round) {
  os << round.model << " ";
  for (std::size_t i = 0; i < round.moves.size(); i++) {
    if (i > 0) os << ",";
    os << round.moves[i];
  }
  return os;
...
std::ostream& operator<<(std::ostream& os, const Game::Paipu& paipu) {
  os << paipu.rule << " ";
  for (std::size_t i = 0; i < paipu.rounds.size(); i++) {
    if (i > 0) os << " ";
    os << paipu.rounds[i];
  }
  os << "\n";
...
void load_model(std::istream& is, Game::Model& model, const Rule& rule) {
  is >> model.qijia >> model.zhuangfeng >> model.jushu >> model.changbang >>
      model.lizhibang >> model.defen;
  {
    std::string paistr;
    is >> paistr;
    model.shan.set(paistr, rule);
    if (is.get() != ' ') throw std::runtime_error("unexpected char");
  }
  ...
  is >> model.player_id >> model.lunban;
}
...
void load_round(std::istream& is, Game::Paipu::Round& round, const Rule& rule) {
  ...
  while (std::getline(ss, move, ',')) {
    round.moves.emplace_back(
        Game::Reply{(Game::Message)to_int(move[0]), move.substr(1)});
  }
}
