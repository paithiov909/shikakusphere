...
int Calsht::calc_lh(const int* t, const int m) const
{
  ...
  LVec ret = [](const RVec& rhs) {
    return LVec(rhs.begin(), rhs.end());
  }(mp1(std::accumulate(t + 1, t + 9, t[0], [](int x, int y) { return 5 * x + y; }), _));
#endif

  add1(ret, mp1(std::accumulate(t + 10, t + 18, t[9], [](int x, int y) { return 5 * x + y; }), _), m);
  add1(ret, mp1(std::accumulate(t + 19, t + 27, t[18], [](int x, int y) { return 5 * x + y; }), _), m);
  add2(ret, mp2(std::accumulate(t + 28, t + 34, t[27], [](int x, int y) { return 5 * x + y; }), _), m);
  return ret[5 + m];
}
