...
bool pick_shunzi(std::vector<std::string>& pai,
                 std::map<std::string, int>& rest, const Rule& rule,
                 std::mt19937_64& mt) {
  ...
  for (int i = 0; i < M; i++) {
    ...
    std::string p[] = {to_string(s, n + 1), to_string(s, n + 2),
                       to_string(s, n + 3)};
    if (rule.hongpai[s_i] && n >= 2 && n <= 4 && dist4(mt) == 0) {
      const auto hongpai = to_string(s, 0);
      if (rest[hongpai] > 0) p[4 - n] = hongpai;
    }
...
// 幺九牌
template <>
bool pick_shunzi<false, true>(std::vector<std::string>& pai,
...
// 緑牌
template <>
bool pick_shunzi<false, false, true>(std::vector<std::string>& pai,
                                     std::map<std::string, int>& rest,
                                     const Rule& rule, std::mt19937_64& mt) {
  const auto p1 = "s2";
  const auto p2 = "s3";
...
// 数牌の種類指定
bool pick_shunzi(const int s_i, std::vector<std::string>& pai,
                 std::map<std::string, int>& rest, const Rule& rule,
                 std::mt19937_64& mt) {
  std::uniform_int_distribution<int> dist4(0, 3);
  constexpr int N = 7;
...
// 牌山から刻子を取り出す
template <bool DUANYAOJIU = false, bool YAOJIU = false, bool ZIPAI = true,
          bool SANYUAN = false, bool LVYISE = false>
bool pick_kezi(std::vector<std::string>& pai, std::map<std::string, int>& rest,
               const Rule& rule, std::mt19937_64& mt) {
  std::uniform_int_distribution<int> dist4(0, 3);
  ...
  for (int i = 0; i < M; i++) {
    ...
    if (s != 'z' && rule.hongpai[s_i] && n == 4 && rest[hongpai] > 0 &&
        dist4(mt) == 0 && rest[p] >= 2) {
      pai.emplace_back(p);
      pai.emplace_back(p);
...
// 幺九牌
template <>
bool pick_kezi<false, true>(std::vector<std::string>& pai,
                            std::map<std::string, int>& rest, const Rule& rule,
                            std::mt19937_64& mt) {
  constexpr int N = 2;
...
// 幺九牌(字牌なし)
template <>
bool pick_kezi<false, true, false>(std::vector<std::string>& pai,
                                   std::map<std::string, int>& rest,
                                   const Rule& rule, std::mt19937_64& mt) {
  constexpr int N = 2;
  constexpr int M = N * 3;
...
// 字牌
template <>
bool pick_kezi<false, false, true, true>(std::vector<std::string>& pai,
                                         std::map<std::string, int>& rest,
                                         const Rule& rule,
                                         std::mt19937_64& mt) {
  constexpr int N = 7;
...
// 三元牌
template <>
bool pick_kezi<false, false, false, true>(std::vector<std::string>& pai,
                                          std::map<std::string, int>& rest,
                                          const Rule& rule,
                                          std::mt19937_64& mt) {
  constexpr int N = 3;
...
// 緑牌
template <>
bool pick_kezi<false, false, true, false, true>(
...
bool pick_kezi(const int s_i, std::vector<std::string>& pai,
               std::map<std::string, int>& rest, const Rule& rule,
               std::mt19937_64& mt) {
  std::uniform_int_distribution<int> dist4(0, 3);
  ...
  for (int i = 0; i < N; i++) {
    ...
    const auto hongpai = to_string(s, 0);
    const auto p = to_string(s, n + 1);
    if ((!ZIPAI || s != 'z') && rule.hongpai[s_i] && n == 4 &&
        rest[hongpai] > 0 && dist4(mt) == 0 && rest[p] >= 2) {
      pai.emplace_back(p);
      pai.emplace_back(p);
      pai.emplace_back(hongpai);
      rest[p] -= 2;
...
template <bool DUANYAOJIU = false, bool YAOJIU = false, bool ZIPAI = true,
          bool SANYUAN = false, bool LVYISE = false>
bool pick_jiangpai(std::vector<std::string>& pai,
                   std::map<std::string, int>& rest, const Rule& rule,
                   std::mt19937_64& mt) {
  std::uniform_int_distribution<int> dist4(0, 3);
  constexpr int N = DUANYAOJIU ? 7 : 9;
...
// 幺九牌
template <>
bool pick_jiangpai<false, true, true>(std::vector<std::string>& pai,
                                      std::map<std::string, int>& rest,
                                      const Rule& rule, std::mt19937_64& mt) {
  constexpr int N = 2;
  constexpr int M = N * 3 + 7;
...
// 幺九牌(字牌なし)
template <>
bool pick_jiangpai<false, true, false>(std::vector<std::string>& pai,
                                       std::map<std::string, int>& rest,
                                       const Rule& rule, std::mt19937_64& mt) {
  constexpr int N = 2;
  constexpr int M = N * 3;
...
// 字牌
template <>
bool pick_jiangpai<false, false, true, true>(std::vector<std::string>& pai,
                                             std::map<std::string, int>& rest,
                                             const Rule& rule,
                                             std::mt19937_64& mt) {
  constexpr int N = 7;
...
// 三元牌
template <>
bool pick_jiangpai<false, false, false, true>(std::vector<std::string>& pai,
                                              std::map<std::string, int>& rest,
                                              const Rule& rule,
                                              std::mt19937_64& mt) {
  constexpr int N = 3;
...
// 緑牌
template <>
bool pick_jiangpai<false, false, true, false, true>(
    std::vector<std::string>& pai, std::map<std::string, int>& rest,
    const Rule& rule, std::mt19937_64& mt) {
  const std::string lvy[] = {"s2", "s3", "s4", "s6", "s8", "z6"};
  constexpr int N = 6;
...
// 数牌の種類指定
template <bool ZIPAI = true>
bool pick_jiangpai(const int s_i, std::vector<std::string>& pai,
                   std::map<std::string, int>& rest, const Rule& rule,
                   std::mt19937_64& mt) {
  std::uniform_int_distribution<int> dist4(0, 3);
  constexpr int N = 9 + (ZIPAI ? 7 : 0);
  ...
  for (int i = 0; i < N; i++) {
    ...
    const auto p = to_string(s, n + 1);
    if ((!ZIPAI || s != 'z') && rule.hongpai[s_i] && n == 4 &&
        rest[hongpai] > 0 && dist4(mt) == 0 && rest[p] >= 1) {
      pai.emplace_back(p);
      pai.emplace_back(hongpai);
      rest[p]--;
      rest[hongpai]--;
...
template <bool DUANYAOJIU = false, bool YAOJIU = false, bool ZIPAI = true,
          bool LVYISE = false, bool JIANGPAI = true>
bool setup_hule(std::vector<std::string>& pai, std::map<std::string, int>& rest,
                const Rule& rule, std::mt19937_64& mt) {
  std::uniform_int_distribution<int> bool_dist(0, 1);
  while (pai.size() < 12) {
    if (bool_dist(mt) == 0) {
      // 順子
      if (pick_shunzi<DUANYAOJIU, YAOJIU, LVYISE>(pai, rest, rule, mt))
        continue;
    }
    // 刻子
    if (!pick_kezi<DUANYAOJIU, YAOJIU, ZIPAI, false, LVYISE>(pai, rest, rule,
                                                             mt))
      return false;
  }
  if (JIANGPAI) {
    // 雀頭
    if (!pick_jiangpai<DUANYAOJIU, YAOJIU, ZIPAI, false, LVYISE>(pai, rest,
                                                                 rule, mt))
      return false;
  }
  return true;
...
// チー
std::string make_chi(const std::string& p1, const std::string& p2,
                     const std::string& p3, std::mt19937_64& mt) {
...
std::string make_gang(const std::string& p1, const std::string& p2,
                      const std::string& p3, const std::string& p,
                      std::mt19937_64& mt) {
...
std::string make_peng_gang(const std::string& p1, const std::string& p2,
                           const std::string& p3,
                           std::map<std::string, int>& rest,
                           std::mt19937_64& mt) {
  std::uniform_int_distribution<int> dist4(0, 3);
  ...
  if (GANG && gang_dist(mt) == 0) {
    ...
    // 赤牌
    if (p1[1] == '5' && p2[1] == '5' && p3[1] == '5' && dist4(mt) == 0 &&
        rest[to_string(p1[0], 0)] > 0) {
      p = to_string(p1[0], 0);
    }
    if (rest[p] > 0) {
      rest[p]--;
...
template <int MIN = 0>
void make_fulou(std::vector<std::string>& pai, std::map<std::string, int>& rest,
                std::vector<std::string>& fulou, std::mt19937_64& mt) {
  ...
  for (int i = 0; i < n_fulou; i++) {
    ...
    if (pai[j] != pai[j + 1])
      ...
      fulou.emplace_back(
          make_peng_gang(pai[j], pai[j + 1], pai[j + 2], rest, mt));
...
template <>
void make_fulou<0>(std::vector<std::string>& pai,
                   std::map<std::string, int>& rest,
                   std::vector<std::string>& fulou, std::mt19937_64& mt) {
...
// 場風
bool setup_zhuangfeng(std::vector<std::string>& pai,
                      std::map<std::string, int>& rest,
                      std::vector<std::string>& fulou, const int zhuangfeng,
                      const Rule& rule, std::mt19937_64& mt) {
...
// 自風
bool setup_menfeng(std::vector<std::string>& pai,
                   std::map<std::string, int>& rest,
                   std::vector<std::string>& fulou, const int l,
                   const Rule& rule, std::mt19937_64& mt) {
...
// 翻牌
bool setup_fanpai(std::vector<std::string>& pai,
                  std::map<std::string, int>& rest,
...
// 平和
bool setup_pinghe(std::vector<std::string>& pai,
                  std::map<std::string, int>& rest, const int zhuangfeng,
                  const int l, const Rule& rule, std::mt19937_64& mt) {
  std::uniform_int_distribution<int> dist4(0, 3);
  ...
  for (int i = 0; i < 4; i++) {
    if (i != zhuangfeng && i != l) kefeng.emplace_back(i);
  }
  constexpr int N = 9;
...
// 断幺九
bool setup_duanyaojiu(std::vector<std::string>& pai,
                      std::map<std::string, int>& rest, const Rule& rule,
                      std::mt19937_64& mt) {
...
// 盃口
bool setup_beikou(std::vector<std::string>& pai,
                  std::map<std::string, int>& rest, const Rule& rule,
                  std::mt19937_64& mt) {
  std::uniform_int_distribution<int> dist4(0, 3);
  ...
  for (int i = 0; i < 7 * 3; i++) {
    ...
    const auto p3 = to_string(s, n + 3);
    const auto hongpai = to_string(s, 0);
    if (rule.hongpai[s_i] && n >= 2 && n <= 4 && rest[hongpai] > 0 &&
        dist4(mt) == 0) {
...
// 一盃口
bool setup_yibeikou(std::vector<std::string>& pai,
                    std::map<std::string, int>& rest, const Rule& rule,
                    std::mt19937_64& mt) {
...
// 三色同順
bool setup_sansetongshun(std::vector<std::string>& pai,
                         std::map<std::string, int>& rest,
                         std::vector<std::string>& fulou, const Rule& rule,
                         std::mt19937_64& mt) {
  std::uniform_int_distribution<int> dist4(0, 3);
  ...
  for (int i = 0; i < 7; i++) {
    ...
    if (rule.hongpai[0] && rule.hongpai[1] && rule.hongpai[2] && n >= 2 &&
        n <= 4 && dist4(mt) == 0) {
      const int hongpai_s = suit_dist(mt);
      const auto hongpai = to_string(suit[hongpai_s], 0);
      if (rest[hongpai] > 0) p[hongpai_s][4 - n] = hongpai;
    }
    if (rest[p[0][0]] > 0 && rest[p[0][1]] > 0 && rest[p[0][2]] > 0 &&
        rest[p[1][0]] > 0 && rest[p[1][1]] > 0 && rest[p[1][2]] > 0 &&
        rest[p[2][0]] > 0 && rest[p[2][1]] > 0 && rest[p[2][2]] > 0) {
      pai.emplace_back(p[0][0]);
      pai.emplace_back(p[0][1]);
      pai.emplace_back(p[0][2]);
      pai.emplace_back(p[1][0]);
      pai.emplace_back(p[1][1]);
      pai.emplace_back(p[1][2]);
      pai.emplace_back(p[2][0]);
      pai.emplace_back(p[2][1]);
      pai.emplace_back(p[2][2]);
      rest[p[0][0]]--;
      rest[p[0][1]]--;
      rest[p[0][2]]--;
      rest[p[1][0]]--;
...
// 一気通貫
bool setup_yiqitongguan(std::vector<std::string>& pai,
                        std::map<std::string, int>& rest,
                        std::vector<std::string>& fulou, const Rule& rule,
                        std::mt19937_64& mt) {
  std::uniform_int_distribution<int> dist4(0, 3);
  ...
  for (int i = 0; i < 3; i++) {
    ...
    const char s = suit[j];
    const auto hongpai = to_string(s, 0);
    const bool is_hongpai =
        rule.hongpai[j] && rest[hongpai] > 0 && dist4(mt) == 0;
    ...
    if (rest[p1] > 0 && rest[p2] > 0 && rest[p3] > 0 && rest[p4] > 0 &&
        rest[p5] > 0 && rest[p6] > 0 && rest[p7] > 0 && rest[p8] > 0 &&
        rest[p9] > 0) {
      pai.emplace_back(p1);
      pai.emplace_back(p2);
      pai.emplace_back(p3);
...
// 混全帯幺九
bool setup_hunquandaiyaojiu(std::vector<std::string>& pai,
                            std::map<std::string, int>& rest,
                            std::vector<std::string>& fulou, const Rule& rule,
                            std::mt19937_64& mt) {
  std::uniform_int_distribution<int> bool_dist(0, 1);
  ...
  if (bool_dist(mt) == 0) {
    ...
    if (!setup_hule<false, true, true, false, false>(pai, rest, rule, mt))
      return false;
    if (!pick_jiangpai<false, false, true, true>(pai, rest, rule, mt))
      return false;
  }
  make_fulou(pai, rest, fulou, mt);
  return true;
...
// 七対子
bool setup_qiduizi(std::vector<std::string>& pai,
                   std::map<std::string, int>& rest, const Rule& rule,
                   std::mt19937_64& mt) {
...
// 対々和
bool setup_duiduihu(std::vector<std::string>& pai,
                    std::map<std::string, int>& rest,
                    std::vector<std::string>& fulou, const Rule& rule,
                    std::mt19937_64& mt) {
...
// 三暗刻
bool setup_sananke(std::vector<std::string>& pai,
                   std::map<std::string, int>& rest,
                   std::vector<std::string>& fulou, const Rule& rule,
                   std::mt19937_64& mt) {
  ...
  if (shunzi <= 1) {
    if (!pick_shunzi(pai, rest, rule, mt)) return false;
    if (shunzi == 1) fulou.emplace_back(make_chi(pai[9], pai[10], pai[11], mt));
  } else {
    if (!pick_kezi(pai, rest, rule, mt)) return false;
...
// 三槓子
bool setup_sangangzi(std::vector<std::string>& pai,
                     std::map<std::string, int>& rest,
                     std::vector<std::string>& fulou, const Rule& rule,
                     std::mt19937_64& mt) {
  std::uniform_int_distribution<int> shunzi_dist(0, 2);
  ...
  if (pai[0][1] == '5' && pai[1][1] == '5' && pai[2][1] == '5' &&
      rest[to_string(p1[0], 0)] > 0)
    p1 = to_string(p1[0], 0);
  if (pai[3][1] == '5' && pai[4][1] == '5' && pai[5][1] == '5' &&
      rest[to_string(p2[0], 0)] > 0)
    p2 = to_string(p2[0], 0);
  if (pai[6][1] == '5' && pai[7][1] == '5' && pai[8][1] == '5' &&
      rest[to_string(p3[0], 0)] > 0)
    p3 = to_string(p3[0], 0);
  if (rest[p1] > 0 && rest[p2] > 0 && rest[p3] > 0) {
    rest[p1]--;
    rest[p2]--;
    ...
    if (shunzi <= 1) {
      ...
      if (!pick_kezi(pai, rest, rule, mt)) return false;
      fulou.emplace_back(
          make_peng_gang<false>(pai[9], pai[10], pai[11], rest, mt));
    }
    if (!pick_jiangpai(pai, rest, rule, mt)) return false;
    if (fulou.size() > 0) {
      pai.erase(pai.begin() + 9, pai.begin() + 12);
...
// 三色同刻
bool setup_sansetongke(std::vector<std::string>& pai,
                       std::map<std::string, int>& rest,
                       std::vector<std::string>& fulou, const Rule& rule,
                       std::mt19937_64& mt) {
  std::uniform_int_distribution<int> dist4(0, 3);
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (n == 4 && rule.hongpai[0] && rule.hongpai[1] && rule.hongpai[2]) {
      ...
      if (is_hongpai) n_pai[hongpai_s]--;
      if (rest[p1] >= n_pai[0] && rest[p2] >= n_pai[1] &&
          rest[p3] >= n_pai[2]) {
...
// 混老頭
bool setup_hunlaotou(std::vector<std::string>& pai,
                     std::map<std::string, int>& rest,
                     std::vector<std::string>& fulou, const Rule& rule,
                     std::mt19937_64& mt) {
  std::uniform_int_distribution<int> bool_dist(0, 1);
...
// 小三元
bool setup_xiaosanyuan(std::vector<std::string>& pai,
                       std::map<std::string, int>& rest,
                       std::vector<std::string>& fulou, const Rule& rule,
                       std::mt19937_64& mt) {
  std::uniform_int_distribution<int> bool_dist(0, 1);
  ...
  if (bool_dist(mt) == 0) {
    if (!pick_shunzi(pai, rest, rule, mt)) return false;
    if (!setup_hule<false, false, true, false, false>(pai, rest, rule, mt))
      return false;
    rest["z5"] += 4;
    rest["z6"] += 4;
    rest["z7"] += 4;
    if (!pick_jiangpai<false, false, false, true>(pai, rest, rule, mt))
      return false;
    make_fulou(pai, rest, fulou, mt);
  } else {
    if (!setup_hule<false, false, true, false, false>(pai, rest, rule, mt))
...
// 混一色
bool setup_hunyise(std::vector<std::string>& pai,
                   std::map<std::string, int>& rest,
                   std::vector<std::string>& fulou, const Rule& rule,
                   std::mt19937_64& mt) {
  std::uniform_int_distribution<int> bool_dist(0, 1);
...
// 純全帯幺九
bool setup_chunquandaiyaojiu(std::vector<std::string>& pai,
                             std::map<std::string, int>& rest,
                             std::vector<std::string>& fulou, const Rule& rule,
                             std::mt19937_64& mt) {
  if (!pick_shunzi<false, true>(pai, rest, rule, mt)) return false;
...
// 二盃口
bool setup_erbeikou(std::vector<std::string>& pai,
                    std::map<std::string, int>& rest,
                    std::vector<std::string>& fulou, const Rule& rule,
                    std::mt19937_64& mt) {
...
// 清一色
bool setup_qingyise(std::vector<std::string>& pai,
                    std::map<std::string, int>& rest,
                    std::vector<std::string>& fulou, const Rule& rule,
                    std::mt19937_64& mt) {
  std::uniform_int_distribution<int> bool_dist(0, 1);
...
// 国士無双
bool setup_guoshiwushuang(std::vector<std::string>& pai,
                          std::map<std::string, int>& rest, const Rule& rule,
                          std::mt19937_64& mt) {
  std::uniform_int_distribution<int> guoshi_dist(0, 11);
  const std::string guoshi[] = {"m1", "m9", "p1", "p9", "s1", "s9", "z1",
                                "z2", "z3", "z4", "z5", "z6", "z7"};
...
// 四暗刻
bool setup_sianke(std::vector<std::string>& pai,
                  std::map<std::string, int>& rest, const Rule& rule,
                  std::mt19937_64& mt) {
  if (!pick_kezi(pai, rest, rule, mt)) return false;
  if (!pick_kezi(pai, rest, rule, mt)) return false;
...
// 大三元
bool setup_dasanyuan(std::vector<std::string>& pai,
                     std::map<std::string, int>& rest,
                     std::vector<std::string>& fulou, const Rule& rule,
                     std::mt19937_64& mt) {
...
// 小四喜
bool setup_xiaosixi(std::vector<std::string>& pai,
                    std::map<std::string, int>& rest,
                    std::vector<std::string>& fulou, const Rule& rule,
                    std::mt19937_64& mt) {
...
// 大四喜
bool setup_dasixi(std::vector<std::string>& pai,
                  std::map<std::string, int>& rest,
                  std::vector<std::string>& fulou, const Rule& rule,
                  std::mt19937_64& mt) {
  std::uniform_int_distribution<int> sixi_dist(0, 3);
...
// 字一色
bool setup_ziyise(std::vector<std::string>& pai,
                  std::map<std::string, int>& rest,
                  std::vector<std::string>& fulou, const Rule& rule,
                  std::mt19937_64& mt) {
  if (!pick_kezi<false, false, true, true>(pai, rest, rule, mt)) return false;
...
// 緑一色
bool setup_lvyise(std::vector<std::string>& pai,
                  std::map<std::string, int>& rest,
                  std::vector<std::string>& fulou, const Rule& rule,
                  std::mt19937_64& mt) {
...
// 清老頭
bool setup_qinglaotou(std::vector<std::string>& pai,
                      std::map<std::string, int>& rest,
                      std::vector<std::string>& fulou, const Rule& rule,
                      std::mt19937_64& mt) {
  if (!pick_kezi<false, true, false>(pai, rest, rule, mt)) return false;
...
// 四槓子
bool setup_sigangzi(std::vector<std::string>& pai,
                    std::map<std::string, int>& rest,
                    std::vector<std::string>& fulou, const Rule& rule,
                    std::mt19937_64& mt) {
  ...
  if (pai[6][1] == '5' && pai[7][1] == '5' && pai[8][1] == '5' &&
      rest[to_string(p3[0], 0)] > 0)
    p3 = to_string(p3[0], 0);
  if (pai[9][1] == '5' && pai[10][1] == '5' && pai[11][1] == '5' &&
      rest[to_string(p4[0], 0)] > 0)
    p4 = to_string(p4[0], 0);
  if (rest[p1] > 0 && rest[p2] > 0 && rest[p3] > 0 && rest[p4] > 0) {
    fulou.emplace_back(make_gang<true>(pai[0], pai[1], pai[2], p1, mt));
    fulou.emplace_back(make_gang<true>(pai[3], pai[4], pai[5], p2, mt));
...
// 九蓮宝燈
bool setup_jiulianbaodeng(std::vector<std::string>& pai,
                          std::map<std::string, int>& rest, const Rule& rule,
                          std::mt19937_64& mt) {
  std::uniform_int_distribution<int> suit_dist(0, 2);
...
// ランダムな役の手牌を作る
bool make_hule(std::vector<std::string>& pai, std::map<std::string, int>& rest,
               std::vector<std::string>& fulou, const int zhuangfeng,
               const int l, const Rule& rule, std::mt19937_64& mt) {
...
// 1枚捨てて聴牌にする
void discard_one(std::vector<std::string>& pai,
                 std::map<std::string, int>& rest,
                 std::vector<std::string>& fulou, std::mt19937_64& mt) {
...
// N向聴にする
void make_n_xiangting(std::vector<std::string>& pai,
                      std::map<std::string, int>& rest,
  ...
  for (auto itr = erace_fulou_i.rbegin(); itr != erace_fulou_i.rend(); ++itr) {
    ...
    auto& pai_ = fulou_pai[fulou_i];
    for (std::size_t j = 0; j < pai_.size(); j++) {
      const auto& p = pai_[j];
      if (j == 3)  // 槓
        rest[p]++;
      else
...
void random_game_state(Game& game, const int n_xiangting, std::mt19937_64& mt) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    while (true) {
      ...
      if (make_hule(pai, rest, fulou, game.zhuangfeng(), l, game.rule(), mt)) {
        // 槓は3まで
        auto n = (int)count_if(
            fulou, [](const auto& m) { return std::regex_match(m, re_gang()); });
        if (n == 4) {
          // 四槓子は副露を1つ戻す
          const auto& m = fulou.back();
          const auto p = std::string(1, m[0]) + m[1];
          rest[p]++;
          for (std::sregex_iterator it(m.begin() + 2, m.end(), re_digit()), end;
               it != end; ++it) {
            const auto p = m[0] + it->str();
            pai.emplace_back(p);
          }
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (i < lunban) he_num[i]++;
    if (he_num[i] > max_he_num) max_he_num = he_num[i];
  }
  if (max_he_num > 14) {
    for (auto& num : he_num) num -= max_he_num - 14;
  }
  ...
  for (int i = 0; i < 4; i++) {
    ...
    for (const auto& p : tingpai(shoupai_, xiangting)) except.emplace(p);
